...
| TC-10 | FR-07 | Unit/System / Functional | Results include pass/fail outcomes and defects include open/closed severities | Build/open dashboard, then repeat with no check data | Counts, pass rate, average timed response, open/high-risk defects and recent-item limits are correct; no-data state does not present 0%/0 ms as measured evidence | Medium |
| TC-11 | FR-08 | Unit/System / Functional | Results/defects include API/endpoint context and text containing commas/quotes | Export results and defects CSV and inspect/download each file | CSV contains QA context and lifecycle fields, uses a timestamped filename, escapes text correctly and still returns a header when the dataset is empty | Medium |
...
`SecureApiQualityPlatform.Tests` includes automated MSTest coverage for expected-status evaluation, response-time boundary handling, authentication-protection logic, CORS evaluation, content-type matching, defect creation/priority mapping, retest closure, dashboard aggregation/empty-data/recent-item behaviour, and CSV export structure/escaping. Execution evidence should be captured from Test Explorer or `dotnet test` after changes are applied.
